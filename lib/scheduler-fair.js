...
function FairScheduler( options ) {
    this.concurrency = 10;
    this.maxTypeShare = 0.80;
    this.maxScanLength = 1000;
    ...
    this.waitingTypesCount = 0; // job types waiting
    this._waiting = {};         // count of waiting jobs by type
    this._running = {};         // count of running jobs by type
}

FairScheduler.prototype.configure = function cofigure( options ) {
    options = options || {};
    if (typeof options.concurrency === 'number') this.concurrency = options.concurrency;
    if (typeof options.maxTypeShare === 'number') this.maxTypeShare = options.maxTypeShare;
    if (typeof options.maxScanLength === 'number') this.maxScanLength = options.maxScanLength;
    return this;
}
...
FairScheduler.prototype.isBlocked = function isBlocked( type ) {
    ...
    // block jobs that have reached their fair share of those waiting
    var fairShare = this._waiting[type] / this.waitingCount;
    if (usingShare >= fairShare) return true;

    // TODO: fair-share among all waiting calls does not protect against ddos
    // TODO: having many waiting types squeezes the fair share to very low,
    // blocking all types ie reverting to in-order and defeating fairness.
    // TODO: a non-running type is not blocked, so a ddos type will always
...
FairScheduler.prototype.gc = function gc( ) {
    ...
    this.typesCount = Object.keys(this._waiting).length;
}

...
function FairScheduler( options ) {
    ...
    this.maxTypeFraction = options.maxTypeFraction > 0 ? options.maxTypeFraction : .8;
    this.runningCount = 0;
    this.waitingCount = 0;
    this.typesCount = 0;
    this._waiting = {};
...
FairScheduler.prototype.select = function select( types ) {
    ...
    var type = types.peek();
    if (type !== undefined && !this.isBlocked(type)) return 0;
...
FairScheduler.prototype.isBlocked = function isBlocked( type ) {
    ...
    var used = this._running[type] / this.concurrency;
    var share = this._waiting[type] / this.waitingCount;
    if (used > Math.round(this.concurrency * this.maxTypeFraction) || used > share) {
        return true;
    }
